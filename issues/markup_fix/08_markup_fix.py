"""
Markup fixer + audit for skd.txt (SKD).
Counterpart of PWG#175, PWK#113, and siblings.

SKD-specific notes: 4 trailing spaces in <F>.

Inputs: ../../../csl-orig/v02/skd/skd.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "skd" / "skd.txt", HERE / "skd.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "skd_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(F)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for skd.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"SKD markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against skd.txt.\n")
        f.write(f"Paired tags: <F>\n")
        f.write(f"Notes: 4 trailing spaces in <F>.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <F>\n\n")
