...
class Dhatu(object):
 def __init__(self,line):
  ...
  self.line = line
  m = re.search(r'^([^:]+):(.*)$',line)
  self.linenum_str = m.group(1)
  self.linenum = int(m.group(1).strip())  # strip removes blanks
  self.line1 = m.group(2)
  self.entry = None
  self.mwverb = None
...
def init_verbs(filein):
 # slurp lines
 with codecs.open(filein,encoding='utf-8',mode='r') as f:
  recs = [Dhatu(line) for line in f if not line.startswith(';')]
...
if __name__=="__main__": 
 ...
 filein1 = sys.argv[3] # Curated Dhatus (VCP-Dhatus.txt
 filein2 = sys.argv[4] # mwvers
 fileout = sys.argv[5] # 
